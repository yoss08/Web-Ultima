...
export function CoachDashboard() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div>
        <h1 className="font-['Playfair_Display'] font-bold text-2xl md:text-4xl text-foreground mb-2 leading-tight">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        <div className="bg-white dark:bg-card p-6 rounded-[28px] border border-gray-100 dark:border-border shadow-sm hover:shadow-md transition-shadow">
          <div className="w-12 h-12 bg-accent/10 rounded-2xl flex items-center justify-center mb-4">
            <Users className="text-accent" size={24} />
          </div>
          <h3 className="text-3xl font-bold text-foreground">{loading ? "..." : stats.studentCount}</h3>
          <p className="text-sm text-muted-foreground font-medium">Total Students</p>
        </div>

        <div className="bg-card p-6 rounded-[28px] border border-border shadow-sm hover:shadow-md transition-shadow">
          <div className="w-12 h-12 bg-accent/10 rounded-2xl flex items-center justify-center mb-4">
            <Calendar className="text-accent" size={24} />
          </div>
          <h3 className="text-3xl font-bold text-foreground">{loading ? "..." : stats.upcomingSessions}</h3>
        ...
        <div className="bg-card p-6 rounded-[28px] border border-border shadow-sm hover:shadow-md transition-shadow">
          ...
          <h3 className="text-3xl font-bold text-foreground">{loading ? "..." : `${stats.performanceAvg}/5`}</h3>
          <p className="text-sm text-muted-foreground font-medium">Session Rating Avg.</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Link 
            ...
            className="flex items-center gap-3 px-6 h-14 bg-accent text-accent-foreground rounded-[18px] font-bold shadow-lg shadow-accent/20 hover:scale-105 active:scale-95 transition-all"
          >
          ...
          >
            <Users size={20} className="text-accent" /> Manage Students
          </Link>

          <Link 
            to="/dashboard/coach/video-review" 
            className="flex items-center gap-3 px-6 h-14 bg-card text-foreground border border-border rounded-[18px] font-bold hover:bg-muted/80 transition-all font-['Poppins'] shadow-sm"
          >
      ...
      <div className="bg-muted/30 border border-dashed border-border rounded-[32px] p-8 md:p-12 text-center">
      ...
      {!loading && stats.studentCount === 0 && (
        <motion.div 
          ...
          className="bg-card border-2 border-dashed border-border rounded-[32px] p-8 md:p-12 text-center"
        >
          ...
          <Link 
            to="/dashboard/coach/schedule" 
            className="inline-flex items-center gap-2 px-8 py-3 bg-accent text-accent-foreground font-bold rounded-xl hover:scale-105 transition-all"
          >
