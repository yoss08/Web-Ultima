...
export function AdminBookingsPage() {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
          ...
          {STATUS_TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs sm:text-sm font-bold transition-all font-['Poppins'] capitalize whitespace-nowrap ${
                  activeTab === tab
      ...
      {(activeTab === "all" || activeTab === "pending") && pendingCount > 0 && nextPendingBooking && (
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-[32px] p-6 shadow-sm overflow-hidden relative">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-8 relative z-10">
            <div className="max-w-xl">
              <p className="text-[10px] uppercase tracking-[0.3em] text-yellow-700 font-bold font-['Poppins']">
                Pending booking summary
              </p>
              <h2 className="text-xl sm:text-2xl font-bold text-foreground mt-2 font-['Playfair_Display']">
                {pendingCount} request{pendingCount > 1 ? "s" : ""} waiting for review
              </h2>
              <p className="text-sm text-muted-foreground mt-2 font-['Poppins'] leading-relaxed">
                Review the next pending booking request and decide whether to accept or decline it directly from the requests page.
              </p>
            </div>
            
            <div className="rounded-3xl bg-card border border-border p-5 w-full lg:w-[400px] shadow-xl shadow-yellow-500/5">
              <div className="flex items-center justify-between mb-4">
                <p className="text-[10px] uppercase tracking-[0.3em] text-muted-foreground font-bold font-['Poppins']">
                  Next Request
                </p>
                <div className="px-2 py-0.5 bg-yellow-500/10 text-yellow-500 rounded-full text-[10px] font-bold">
                  Priority
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-2xl border border-border/50">
                  <div className="w-10 h-10 rounded-xl bg-accent/10 flex items-center justify-center text-accent font-bold">
                    {nextPendingBooking.profiles?.full_name?.[0] ?? "U"}
                  </div>
                  <div>
                    <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest font-['Poppins']">Player</p>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-muted/30 rounded-2xl border border-border/50">
                    <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest font-['Poppins']">Court</p>
                    <p className="font-bold text-foreground text-xs font-['Poppins'] truncate">{nextPendingBooking.courts?.name || "—"}</p>
                  </div>
                  <div className="p-3 bg-muted/30 rounded-2xl border border-border/50">
                    <p className="text-[10px] text-muted-foreground uppercase font-bold tracking-widest font-['Poppins']">Schedule</p>
                    <p className="font-bold text-foreground text-[10px] font-['Poppins'] truncate">
                      {nextPendingBooking.booking_date || "—"}
                    </p>
                  </div>
                </div>
                
                <div className="p-3 bg-muted/30 rounded-2xl border border-border/50 flex items-center gap-2">
                  <Clock size={12} className="text-accent" />
                  <p className="font-bold text-foreground text-xs font-['Poppins']">
                    {nextPendingBooking.time_slot || "—"}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-6 flex gap-3">
                <button
                  ...
                  className={`flex-1 px-4 py-3 rounded-2xl transition-all font-bold text-sm font-['Poppins'] flex items-center justify-center gap-2 ${nextPendingLoading ? 'opacity-50 cursor-not-allowed bg-emerald-400/10 text-emerald-400' : 'bg-emerald-400 text-white shadow-lg shadow-emerald-400/20 hover:scale-[1.02] active:scale-95'}`}
                >
                  {nextPendingLoading ? <Loader2 size={16} className="animate-spin" /> : <CheckCircle2 size={16} />}
                  Accept
                </button>
                <button
                  onClick={() => handleReject(nextPendingBooking.id)}
                  disabled={nextPendingLoading}
                  className={`flex-1 px-4 py-3 rounded-2xl transition-all font-bold text-sm font-['Poppins'] flex items-center justify-center gap-2 ${nextPendingLoading ? 'opacity-50 cursor-not-allowed bg-red-500/10 text-red-500' : 'bg-red-500/10 text-red-500 border border-red-500/20 hover:bg-red-500 hover:text-white'}`}
                >
                  {nextPendingLoading ? <Loader2 size={16} className="animate-spin" /> : <XCircle size={16} />}
                  Decline
                </button>
              </div>
            </div>
          ...
          <div className="p-6 sm:p-8">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {filtered.map((booking) => {
                ...
                const statusColor = 
                  normalizeStatus(booking.status) === 'confirmed' ? 'text-emerald-400 bg-emerald-400/10 border-emerald-400/20' :
                  normalizeStatus(booking.status) === 'cancelled' || normalizeStatus(booking.status) === 'declined' ? 'text-red-500 bg-red-500/10 border-red-500/20' :
                ...
                return (
                  <motion.div
                    ...
                    className="group bg-card border border-border rounded-[32px] overflow-hidden transition-all duration-300 shadow-sm hover:shadow-md flex flex-col"
                  >
                    <div className="p-6 flex flex-col h-full">
                      {/* Top Bar */}
                      <div className="flex items-center justify-between mb-6">
                        <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border font-['Poppins'] ${statusColor}`}>
                          {normalizeStatus(booking.status) === 'confirmed' ? 'Accepted' : 
                           normalizeStatus(booking.status) === 'cancelled' ? 'Declined' : 
                           booking.status}
                        </span>
                      ...
                      <div className="flex items-center gap-4 mb-6">
                        <div className="w-12 h-12 rounded-2xl bg-accent/10 flex items-center justify-center text-accent font-black text-lg">
                          {booking.profiles?.full_name?.[0] ?? <User size={20} />}
                        </div>
                        <div className="min-w-0">
                          <h3 
                            className="font-bold text-foreground truncate font-['Poppins'] text-lg hover:text-accent cursor-pointer transition-colors"
                            onClick={() => setSelectedBooking(booking)}
                          >
                            {booking.profiles?.full_name ?? 'Unknown Player'}
                          </h3>
                          <div className="flex items-center gap-2 text-muted-foreground text-xs font-['Poppins']">
                            <MapPin size={12} className="text-accent" />
                            <span className="truncate">{booking.courts?.name ?? 'Unassigned Court'}</span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="bg-muted/30 rounded-2xl p-4 mb-6 border border-border/50">
                        <div className="flex items-center justify-between text-xs font-['Poppins']">
                          ...
                          <span className="text-foreground font-bold">{new Date(booking.start_time).toLocaleDateString()}</span>
                        </div>
                        <div className="h-px bg-border/50 my-3" />
                        <div className="flex items-center justify-between text-xs font-['Poppins']">
                          <span className="text-muted-foreground font-medium">Sessions</span>
                          <span className="text-foreground font-bold">1 Session</span>
                        </div>
                      </div>
                        ...
                        {isPending ? (
                          <>
                            <button
                              ...
                              disabled={isActing}
                              className="flex-1 h-11 rounded-xl bg-emerald-400 text-white font-bold text-xs flex items-center justify-center gap-2 hover:opacity-90 transition-all shadow-lg shadow-emerald-400/20 disabled:opacity-50"
                            >
                            ...
                            <button
                              ...
                              disabled={isActing}
                              className="flex-1 h-11 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 font-bold text-xs flex items-center justify-center gap-2 hover:bg-red-500 hover:text-white transition-all disabled:opacity-50"
                            >
                              <XCircle size={16} />
                              REJECT
                            </button>
                          ...
                          <>
                            {booking.match ? (
                              <div className="flex-1 h-11 rounded-xl bg-blue-400/10 text-blue-400 font-bold text-xs flex items-center justify-center gap-2 border border-blue-400/20">
                                <Zap size={16} /> MATCH LIVE
                              </div>
                            ) : (
                              <button
                                onClick={() => handleStartMatch(booking)}
                                disabled={isActing}
                                className="flex-1 h-11 rounded-xl bg-blue-400 text-white font-bold text-xs flex items-center justify-center gap-2 hover:opacity-90 transition-all shadow-lg shadow-blue-400/20 disabled:opacity-50"
                              >
                                {isActing ? <Loader2 size={16} className="animate-spin" /> : <Zap size={16} />}
                                START MATCH
                              </button>
                            )}
                            <button
                              ...
                              disabled={isActing}
                              className="w-11 h-11 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all"
                            >
                              <XCircle size={18} />
                            </button>
                          </>
                        ) : (
                          <div className="w-full py-3 text-center text-xs font-bold text-muted-foreground/40 font-['Poppins'] uppercase tracking-widest border border-dashed border-border rounded-xl">
                            Request Processed
                          </div>
                        )}
                      </div>
              ...
              <div className="space-y-6 font-['Poppins']">
                ...
                <div className="pt-2">
                  {selectedBooking.status === 'pending' ? (
                    ...
                    <div className="flex gap-3">
                      <button
                        onClick={() => { setCancelTarget(selectedBooking); setSelectedBooking(null); }}
                        className="flex-1 py-4 bg-red-500/10 text-red-500 border border-red-500/20 rounded-2xl font-bold hover:bg-red-500 hover:text-white transition-all flex items-center justify-center gap-2"
                      >
        ...
        {cancelTarget && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
            <motion.div
              ...
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              className="bg-card border border-border w-full max-w-sm rounded-[32px] p-8 shadow-2xl relative overflow-hidden"
            >
              ...
              <div className="flex flex-col items-center text-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-bold text-foreground font-['Playfair_Display']">Cancel Booking?</h2>
                  <p className="text-sm text-muted-foreground mt-2 font-['Poppins']">
                    You are about to cancel the confirmed booking for{" "}
                    <span className="font-bold text-foreground">
                      {cancelTarget.profiles?.full_name || "this player"}
                    </span>{" "}
                    on <span className="font-bold text-foreground">{cancelTarget.courts?.name || "—"}</span>.
                    <br className="my-1" />
                ...
                <div className="flex gap-3 w-full mt-2">
                  <button
                    ...
                    className="flex-1 py-3 bg-muted text-foreground rounded-2xl font-bold font-['Poppins'] hover:bg-muted/80 transition-all disabled:opacity-50"
                  >
                    Keep It
                  </button>
                  <button
                    ...
                    disabled={actionLoading === cancelTarget.id}
                    className="flex-1 py-3 bg-red-500 text-white rounded-2xl font-bold font-['Poppins'] hover:bg-red-600 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
