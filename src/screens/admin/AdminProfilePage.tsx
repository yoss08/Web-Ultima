...
type Tab = "Account Profile" | "Notification history";
    ...
    try {
      ...
      if (clubIds.length > 0) {
        ...
        const { data: courts } = await supabase
          ...
          .select("id")
          .in("club_id", clubIds);
        setCourtsCount(courts?.length ?? 0);
        ...
        const { data: coaches } = await supabase
          ...
          .in("club_id", clubIds)
          .eq("role", "coach");
        setCoachesCount(coaches?.length ?? 0);
      }
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20 animate-in fade-in duration-500">
      ...
      <div className="relative overflow-hidden rounded-[28px] mb-6 bg-gradient-to-b from-accent to-background">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-2">
          <div className="flex items-center gap-1.5 bg-black/10 rounded-full px-3 py-1.5">
            ...
            <span className="text-black text-xs font-bold uppercase tracking-wider">Club Admin</span>
          </div>
        </div>
          ...
          <div className="flex flex-col items-center md:items-start flex-1 w-full mt-4 md:mt-2">
            ...
            <div className="flex flex-col md:flex-row items-center md:items-end w-full gap-6 md:gap-12 mt-5 md:mt-8">
              ...
              <div className="flex items-center gap-0 w-full max-w-xs md:max-w-md px-6 md:px-0">
                <StatChip icon={<Building2 className="w-4 h-4 md:w-5 md:h-5 text-black" />} value={clubsCount} label="CLUBS" />
                <div className="w-px h-9 md:h-12 bg-black/20 mx-2 md:mx-4" />
                <StatChip icon={<Grid3X3 className="w-4 h-4 md:w-5 md:h-5 text-black" />} value={courtsCount} label="COURTS" />
                <div className="w-px h-9 md:h-12 bg-black/20 mx-2 md:mx-4" />
                <StatChip icon={<Users className="w-4 h-4 md:w-5 md:h-5 text-black" />} value={coachesCount} label="COACHES" />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex bg-card border border-border rounded-[18px] p-1 mb-6 overflow-x-auto no-scrollbar">
        {(["Account Profile", "Notification history"] as Tab[]).map((tab) => (
          <button
            key={tab}
        ...
        <motion.div key={activeTab} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.25 }}>
          {activeTab === "Account Profile" && (
            <div className="bg-card border border-border rounded-[20px] p-8">
              <h3 className="text-xl font-bold mb-4">Admin Dashboard Privileges</h3>
              <ul className="space-y-4">
                <li className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center text-accent">
                    <Building2 className="w-5 h-5" />
                  </div>
                  <span>Manage your club settings and profile details.</span>
                </li>
                <li className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center text-accent">
                    <Grid3X3 className="w-5 h-5" />
                  </div>
                  <span>Create and configure courts, set pricing, and set schedules.</span>
                </li>
                <li className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-accent/10 flex items-center justify-center text-accent">
                    <ClipboardList className="w-5 h-5" />
                  </div>
                  <span>Review and manage all incoming booking requests.</span>
                </li>
              </ul>
            </div>
          )}
